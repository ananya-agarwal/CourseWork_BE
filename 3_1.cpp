...
{
	//dy and dx nikalo usse m = dy/dx se m nikalo
	//if m<=1, x++, y=y+m
	//if m>1, y++, x=x+1/m
	...
	float step = 0;
	//fabs folating values ka abs nikalta ha
	if (fabs(dx) > fabs(dy))
		//agar dx bada hua toh m<=1 case 
		step = dx;
	else
		step = dy;
	...
	float xincr = dx / step;
	//now dx/step = dx/dx
	//for m>1 case, xincr=dx/dy = 1/m
	float yincr = dy / step;
	//yincr = dy/dx kyuki step = dx
